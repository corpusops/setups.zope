...
from test.cgwb import PRODUCT_DEPENDENCIES

logger = logging.getLogger('test.cgwb / setuphandler')
...
def setupVarious(context):
    ...
    if context.readDataFile('test.cgwb_various.txt') is None:
        return
...
def setupQi(context):
    ...
    if context.readDataFile('test.cgwb_qi.txt') is None:
        return

    portal = context.getSite()
    portal_quickinstaller = getToolByName(portal, 'portal_quickinstaller')
    #portal_setup = getToolByName(portal, 'portal_setup')
    logger = logging.getLogger('test.cgwb.Install')
